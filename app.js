...
app.post('/register', async(req, res) => {
    const {name, sunname, setor, usersname, password} = req.body;
    const hashedPassword = await bcryptjs.hash(password, 10);

    const sql = 'INSERT INTO Users (name, sunname, setor, usersname, password) VALUES (?, ?, ?, ?, ?)';
    db.query(sql, [name, sunname, setor, usersname, hashedPassword], (err, result) => {
        if(err){
...
app.post('/login', (req, res) => {
    ...
    const sql = 'SELECT * FROM Users WHERE usersname = ?';
    db.query(sql, [usersname], async(err, result) =>  {
        if(err){
